...
# Новая функция инициализации: равномерное распределение по свободным клеткам
def init_uniform(high, width):
    ...
    for (i, j) in free_cells:
        p[i, j] = 1.0 / n
    # Реальное положение робота выбираем случайно
    real_i, real_j = rd.choice(free_cells)
    return p, [real_i, real_j]

p, real = init_uniform(high, width)
print("Начальное равномерное распределение вероятностей. Реальная позиция:", real)
...
# ================== Класс Robot (без изменений, кроме добавления методов для навигации) ==================
...
def get_direction_to_target(from_i, from_j, to_i, to_j):
    """Возвращает предпочтительное направление ('up','down','left','right') для движения от (from_i,from_j) к (to_i,to_j)."""
    di = to_i - from_i
    dj = to_j - from_j
    if abs(di) > abs(dj):
...
def orientation_to_index(orient):
    return rOrientation_keys.index(orient)

def angle_difference(current_orient, target_orient):
    """Возвращает минимальное количество поворотов (положительное - направо, отрицательное - налево) для достижения target_orient."""
    ci = orientation_to_index(current_orient)
    ti = orientation_to_index(target_orient)
    diff = (ti - ci) % 4
    if diff > 2:
        diff = diff - 4  # теперь в диапазоне [-2, 2]
    return diff

# ================== Визуализация Pygame с двумя картами ==================
...
# ---------- Левая карта (основная) ----------
def draw_main_map(screen, ground_sprites, drone_sprites, robot, prediction, start_pos, goal_pos, offset_x):
    """Рисует основную карту с дроном, рамками, предсказанием, стартом (жёлтый) и целью (оранжевый)."""
    ...
    # Рисуем стартовую позицию (жёлтый квадрат)
    si, sj = start_pos
    x_start = offset_x + (sj + BORDER) * CELL_SIZE
    y_start = (si + BORDER) * CELL_SIZE
    rect_start = pygame.Rect(x_start, y_start, CELL_SIZE, CELL_SIZE)
    pygame.draw.rect(screen, (255, 255, 0), rect_start, 3)  # жёлтый

    # Рисуем целевую позицию (оранжевый квадрат)
    gi, gj = goal_pos
    x_goal = offset_x + (gj + BORDER) * CELL_SIZE
    y_goal = (gi + BORDER) * CELL_SIZE
    rect_goal = pygame.Rect(x_goal, y_goal, CELL_SIZE, CELL_SIZE)
    pygame.draw.rect(screen, (255, 165, 0), rect_goal, 3)  # оранжевый
...
def draw_prob_map(screen, p, prediction, start_pos, goal_pos, offset_x):
    """Рисует карту вероятностей с красными оттенками, цифрами, синим квадратом предсказания, а также жёлтым стартом и оранжевой целью."""
    rect = pygame.Rect(offset_x, 0, MAP_WIDTH, MAP_HEIGHT)
    screen.fill((0, 0, 0), rect)
    ...
    # Жёлтый квадрат старта
    si, sj = start_pos
    x_start = offset_x + (sj + BORDER) * CELL_SIZE
    y_start = (si + BORDER) * CELL_SIZE
    rect_start = pygame.Rect(x_start, y_start, CELL_SIZE, CELL_SIZE)
    pygame.draw.rect(screen, (255, 255, 0), rect_start, 3)

    # Оранжевый квадрат цели
    gi, gj = goal_pos
    x_goal = offset_x + (gj + BORDER) * CELL_SIZE
    y_goal = (gi + BORDER) * CELL_SIZE
    rect_goal = pygame.Rect(x_goal, y_goal, CELL_SIZE, CELL_SIZE)
    pygame.draw.rect(screen, (255, 165, 0), rect_goal, 3)

    # Синий квадрат предсказания
    pi, pj = prediction
    if world[pi, pj] != 'wall':
        x_pred = offset_x + (pj + BORDER) * CELL_SIZE
        y_pred = (pi + BORDER) * CELL_SIZE
        rect_pred = pygame.Rect(x_pred, y_pred, CELL_SIZE, CELL_SIZE)
        pygame.draw.rect(screen, (0, 0, 255), rect_pred, 3)
...
def draw_future_scan_targets(screen, robot, offset_x):
    ...
    for pi, pj in positions:
        if 0 <= pi < high and 0 <= pj < width:
            ...
            rect = pygame.Rect(x, y, CELL_SIZE, CELL_SIZE)
            pygame.draw.rect(screen, (128, 0, 128), rect, 3)
...
def draw_all(screen, ground_sprites, drone_sprites, robot, p, prediction, mode,
             start_pos, goal_pos, false_count=None, step=None, show_future=False):
    # Левая карта (основная) - смещение 0
    draw_main_map(screen, ground_sprites, drone_sprites, robot, prediction, start_pos, goal_pos, 0)
    # Правая карта (вероятности) - смещение MAP_WIDTH + GAP
    draw_prob_map(screen, p, prediction, start_pos, goal_pos, MAP_WIDTH + GAP)
...
# ================== Автоматический режим (новая версия) ==================

def run_auto_mode(screen, ground_sprites, drone_sprites, robot):
    global p
    clock = pygame.time.Clock()

    # Генерируем стартовую и целевую точки (не стены и не совпадают)
    free_cells = [(i, j) for i in range(high) for j in range(width) if world[i, j] != 'wall']
    start_pos = tuple(robot.position())  # текущее реальное положение робота
    # Убираем старт из возможных целей
    other_cells = [cell for cell in free_cells if cell != start_pos]
    if not other_cells:
        print("Ошибка: нет других свободных клеток для цели")
        return
    goal_pos = rd.choice(other_cells)

    print(f"Старт: {start_pos}, Цель: {goal_pos}")

    step_counter = 0
    max_steps = 200  # ограничение на число шагов
    reached = False

    while step_counter < max_steps and not reached:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
                sys.exit()

        # Сначала выполняем сенсоры (чтобы уточнить положение)
        ...
        prediction = argmax(p)
        print(f"Шаг {step_counter}: предсказание={prediction}, реальное={robot.position()}")

        # Проверка, достигнута ли цель (по реальному положению)
        if tuple(robot.position()) == goal_pos:
            reached = True
            print("Цель достигнута!")
            # Покажем финальный кадр
            draw_all(screen, ground_sprites, drone_sprites, robot, p, prediction, "auto",
                     start_pos, goal_pos, false_count=None, step=step_counter, show_future=False)
            clock.tick(2)
            pygame.time.delay(1000)
            break

        # Принятие решения о следующем действии
        # Вычисляем направление к цели от наиболее вероятного положения
        target_dir = get_direction_to_target(prediction[0], prediction[1], goal_pos[0], goal_pos[1])
        current_orient = robot.str_orientation()
        diff = angle_difference(current_orient, target_dir)

        # Если нужно повернуть, делаем поворот
        if diff != 0:
            if diff > 0:
                # поворот направо
                p = robot.turn_right()
                action = "поворот направо"
            else:
                p = robot.turn_left()
                action = "поворот налево"
        else:
            # уже смотрим в нужную сторону - двигаемся вперёд
            p = robot.dir_move(p, 1)
            action = "движение вперёд"

        print(f"Действие: {action}")

        # Обновляем отображение после действия (показываем фиолетовые рамки будущих сенсоров)
        prediction_after = argmax(p)
        draw_all(screen, ground_sprites, drone_sprites, robot, p, prediction_after, "auto",
                 start_pos, goal_pos, false_count=None, step=step_counter, show_future=True)
        clock.tick(2)
    ...
    if not reached:
        print("Не удалось достичь цели за максимальное число шагов.")
    print("Автоматический режим завершён.")
...
def run_manual_mode(screen, ground_sprites, drone_sprites, robot):
    ...
    running = True
    show_future = True
    # В ручном режиме старт и цель не заданы, поэтому передаём фиктивные (например, (0,0))
    dummy_start = (0,0)
    dummy_goal = (0,0)
    while running:
        prediction = argmax(p)
        draw_all(screen, ground_sprites, drone_sprites, robot, p, prediction, "manual",
                 dummy_start, dummy_goal, show_future=show_future)
